 ...
 */
function figureRenderer(md, resolver) {
  return function figureRender(tokens, idx) {
    const all = tokens[idx];
    const imgstr = md.utils.escapeHtml(all.fig)
    const m = imgstr.match(/^(?<name>.*?)(?::(?<pos>.*))?$/)
    const name = m.groups.name
    const pos = m.groups.pos || "right"
    const full = resolver(name)
    const size = pos == "center" ? 450 : 280
    const href = resolver(name, size)
    const id = all.figid ? ` id="figure-${md.utils.escapeHtml(all.figid)}"` : '';
    const fig = all.fig ? `src="${href}"` : '';
    ...
    const figure = `<figure${id} class="fig-${pos}"><a href="${full}"><img ${fig} ${figalt}/></a>${figcaption}</figure>`;
...
module.exports =resolver=> {
  function figurePlugin(md) {
    md.renderer.rules.figure = figureRenderer(md, resolver);
